...
namespace NXT
{
    ...
    {
        Idle = 0x00,    // No regulation will be enabled
        Motor_Speed = 0x01,     // Power control will be enabled on specified output
    ...
    public class CDeviceInfo
    {
        ...
        public string BTAddress
        {
            ...
            {
                Update();
                return String.Format("{0:x2}:{1:x2}:{2:x2}:{3:x2}:{4:x2}:{5:x2}:{6:x2}", device_info[15], device_info[16], device_info[17], device_info[18], device_info[19], device_info[20], device_info[21]);
            }
        }
        ...
        public uint FreeUserFlash
        {
            ...
            {
                Update();
                return (uint)(device_info[25] + (device_info[26] << 8) + (device_info[27] << 16) + (device_info[28] << 24));
            }
        }
    }
    ...
    public class Robot
    {
        ...
        public Robot(string ComPort)
        {
            ...
            if (!Connect(ComPort))
            {
                Console.WriteLine("Could not connect to NXT");
                Environment.Exit(0);
                return;
            }
            comPort = ComPort;
        ...
        public void StartMotorA(int speed) { SetOutputState(speed, MotorPort.MotorA); }
        public void StartMotorB(int speed) { SetOutputState(speed, MotorPort.MotorB); }
        public void StartMotorC(int speed) { SetOutputState(speed, MotorPort.MotorC); }
        
        public void StopMotorA() { SetOutputState(0, MotorPort.MotorA); }
        public void StopMotorB() { SetOutputState(0, MotorPort.MotorB); }
        public void StopMotorC() { SetOutputState(0, MotorPort.MotorC); }

        private void SetOutputState(int speed, MotorPort motor)
        {
            byte[] message = new byte[13];
            byte[] ByteSpeed = BitConverter.GetBytes(speed);
            byte[] ByteTurnRatio = BitConverter.GetBytes(100);

            // Populate message
            message[0] = 0x00;  // Feedback required
            message[1] = 0x04;
            message[2] = (byte)motor;  // Select motor port
            message[3] = ByteSpeed[0];
            message[4] = 0x01;
            message[5] = 0x00;
            message[6] = ByteTurnRatio[0];
            message[7] = (byte)RunState.Running;
            message[8] = 0x00;
            message[9] = 0x00;
            message[10] = 0xFF;
            message[11] = 0x00;
        ...
        private bool checkResponce(ref byte StatusByte)
        {
            string error = "";
            switch (StatusByte)
            { 
